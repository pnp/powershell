    ...
    public class GetPnPAzureADAppSitePermission : PnPGraphCmdlet
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            if (siteId != Guid.Empty)
            {
                ...
                {
                    // all permissions
                    var results = Utilities.REST.RestHelper.GetAsync<RestResultCollection<AzureADAppPermissionInternal>>(HttpClient, $"https://{PnPConnection.CurrentConnection.GraphEndPoint}/v1.0/sites/{siteId}/permissions", AccessToken).GetAwaiter().GetResult();
                ...
                {
                    var results = Utilities.REST.RestHelper.GetAsync<AzureADAppPermissionInternal>(HttpClient, $"https://{PnPConnection.CurrentConnection.GraphEndPoint}/v1.0/sites/{siteId}/permissions/{PermissionId}", AccessToken).GetAwaiter().GetResult();
