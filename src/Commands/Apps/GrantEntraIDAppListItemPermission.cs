...
{
    [Cmdlet(VerbsSecurity.Grant, "PnPEntraIDAppListItemPermission")]
    [RequiredApiDelegatedPermissions("graph/Sites.FullControl.All")]
    [OutputType(typeof(AzureADAppPermission))]
    public class GrantPnPEntraIDAppListItemPermission : PnPGraphCmdlet
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            LogDebug($"Granting App {AppId} the permission{(roles.Length != 1 ? "s" : "")} {string.Join(", ", roles)} on item {ListItem} in list {listId}");

            var result = Utilities.REST.RestHelper.Post<EntraIDAppListPermissionInternal>(
                Connection.HttpClient,
                $"https://{Connection.GraphEndPoint}/beta/sites/{siteId}/lists/{listId}/items/{ListItem}/permissions",
                AccessToken,
                payload);
