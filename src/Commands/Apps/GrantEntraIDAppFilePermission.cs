...
{
    [Cmdlet(VerbsSecurity.Grant, "PnPEntraIDAppFilePermission")]
    [RequiredApiDelegatedPermissions("graph/Sites.FullControl.All")]
    [OutputType(typeof(AzureADAppPermission))]
    public class GrantPnPEntraIDAppFilePermission : PnPGraphCmdlet
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            LogDebug($"Granting App {AppId} the permission{(roles.Length != 1 ? "s" : "")} {string.Join(", ", roles)} on drive item {driveItemId} in drive {driveId}");

            var result = Utilities.REST.RestHelper.Post<EntraIDAppDrivePermissionInternal>(
                Connection.HttpClient,
                $"https://{Connection.GraphEndPoint}/beta/drives/{driveId}/items/{driveItemId}/permissions",
                AccessToken,
                payload);
