...
{
    [Cmdlet(VerbsCommon.Remove, "PnPTodoTaskLinkedResource")]
    [RequiredApiDelegatedPermissions("graph/Tasks.ReadWrite")]
    [ApiNotAvailableUnderApplicationPermissions]
    public class RemoveTodoTaskLinkedResource : PnPGraphCmdlet
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            var graphResult = ToDoUtility.DeleteLinkedResource(GraphRequestHelper, url, listId, Task.Id, Identity);

            if (graphResult.StatusCode == System.Net.HttpStatusCode.NoContent)
            {
                LogDebug("Todo task linked resource deleted successfully");
            }
            else
            {
                throw new PSArgumentException("Todo task linked resource could not be deleted", nameof(Identity));
            }
        }
    }
}
