...
{
    [Cmdlet(VerbsCommon.Set, "PnPMessageCenterAnnouncementAsArchived", DefaultParameterSetName = ParameterSet_ALL)]
    ...
    public class SetMessageCenterAnnouncementAsArchived : PnPGraphCmdlet
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            {
                WriteObject(ServiceHealthUtility.SetServiceUpdateMessageAsArchivedByIdAsync(Identities, HttpClient, AccessToken).GetAwaiter().GetResult(), true);
            }
            else
            {
