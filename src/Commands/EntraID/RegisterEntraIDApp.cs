...
namespace PnP.PowerShell.Commands.EntraID
{
    ...
    public class RegisterEntraIDApp : BasePSCmdlet, IDynamicParameters
    {
        ...
        private bool AppExists(string appName, HttpClient httpClient, string token)
        {
            ...
            var azureApps = RestHelper.GetAsync<RestResultCollection<App>>(httpClient, $@"https://{PnP.Framework.AuthenticationManager.GetGraphEndPoint(AzureEnvironment)}/v1.0/applications?$filter=displayName eq '{appName}'&$select=Id", token).GetAwaiter().GetResult();
        ...
        private App CreateApp(string loginEndPoint, HttpClient httpClient, string token, X509Certificate2 cert, string redirectUri, List<PermissionScope> scopes)
        {
            ...
            var azureApp = RestHelper.PostAsync<App>(httpClient, $"https://{AuthenticationManager.GetGraphEndPoint(AzureEnvironment)}/v1.0/applications", token, payload).GetAwaiter().GetResult();
            if (azureApp != null)
            {
        ...
        private void StartConsentFlow(string loginEndPoint, App azureApp, string redirectUri, string token, HttpClient httpClient, PSObject record, CmdletMessageWriter messageWriter, List<PermissionScope> scopes)
        {
        ...
        private void SetLogo(App azureApp, string token)
        {
            ...
            if (File.Exists(LogoFilePath))
            {
                try
                {
                    WriteVerbose("Setting the logo for the Entra ID app");
