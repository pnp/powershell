    ...
    public sealed class GroupPipeBind
    {
        ...
        internal ISharePointGroup GetGroup(PnPContext context)
        {
            ...
            if (Group != null)
            {
                ...
                group = context.Web.SiteGroups.AsQueryable().Where(g => g.Id == Group.Id).FirstOrDefault();
            }
            ...
            {
                group = context.Web.SiteGroups.AsQueryable().Where(g => g.Id == Id).FirstOrDefault();
            }
            else if (!string.IsNullOrEmpty(Name))
            {
                group = context.Web.SiteGroups.AsQueryable().Where(g => g.Title == Name && g.LoginName == Name).FirstOrDefault();
            }
            return group;
        }
