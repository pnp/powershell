    ...
    public sealed class GroupPipeBind
    {
        ...
        internal ISharePointGroup GetGroup(PnPContext context)
        {
            ...
            if (Group != null)
            {
                ...
                group = ((IQueryable<ISharePointGroup>)context.Web.SiteGroups).Where(g => g.Id == Group.Id).FirstOrDefault();
            }
            ...
            {
                group = ((IQueryable<ISharePointGroup>)context.Web.SiteGroups).Where(g => g.Id == Id).FirstOrDefault();
            }
            else if (!string.IsNullOrEmpty(Name))
            {
                group = ((IQueryable<ISharePointGroup>)context.Web.SiteGroups).Where(g => g.Title == Name && g.LoginName == Name).FirstOrDefault();
            }
            return group;
        }
