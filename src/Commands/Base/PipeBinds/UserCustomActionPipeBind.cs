            ...
            if (_userCustomAction != null)
            {
                switch (_userCustomAction.Scope)
                {
                    case Microsoft.SharePoint.Client.UserCustomActionScope.Web:
                        {
                            return new List<IUserCustomAction> { context.Web.UserCustomActions.AsQueryable().Where(ca => ca.Id == _userCustomAction.Id).FirstOrDefault() };
                        }
                        case Microsoft.SharePoint.Client.UserCustomActionScope.Site:
                        {
                            return new List<IUserCustomAction> { context.Site.UserCustomActions.AsQueryable().Where(ca => ca.Id == _userCustomAction.Id).FirstOrDefault() };
                        }
