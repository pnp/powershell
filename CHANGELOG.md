...
- Added the Microsoft Graph permissions released since the list shipping with the module was last refreshed, taking the delegated permissions from 294 to 797 and the application permissions from 223 to 707. `Register-PnPEntraIDApp -GraphApplicationPermissions` and `-GraphDelegatePermissions` accept all of them now. [#4178](https://github.com/pnp/powershell/issues/4178)
- Added the missing SharePoint permissions, taking the delegated permissions from 18 to 29 and the application permissions from 9 to 20. Newly available are `Migration.Read.All`, `Migration.ReadWrite.All`, `SharePointCrossTenantMigration.Manage.All`, `SharePointCrossTenantMigration.Read.All`, `SharePointTenantSettings.Read.All`, `SharePointTenantSettings.ReadWrite.All`, `SitesMetadataAdmin.Read.All`, `SitesMetadataAdmin.ReadWrite.All`, `TenantReports.Read.All`, `TenantReports.ReadWrite.All`, `OneDrive.Provision.All`, `EnterpriseResource.Write` and `ProjectWebAppReporting.Read`. [#4178](https://github.com/pnp/powershell/issues/4178)
- Added `-ExchangeApplicationPermissions`, `-ExchangeDelegatePermissions`, `-PowerBIApplicationPermissions`, `-PowerBIDelegatePermissions`, `-DataverseDelegatePermissions`, `-PowerAppsDelegatePermissions` and `-AzureServiceManagementDelegatePermissions` to `Register-PnPEntraIDApp`, covering Exchange Online, Power BI, Dataverse, PowerApps and Azure Resource Manager. The last three expose delegated permissions only, hence no application permission parameter for them. Added `-ResourcePermissions` to the same cmdlet to request permissions on any other API, and on permissions the dedicated parameters do not offer. The permissions of these resources are read from the tenant instead of a list shipping with the module. [#4178](https://github.com/pnp/powershell/issues/4178)

### Changed
- Changed which permissions `Register-PnPEntraIDApp` and `Register-PnPEntraIDAppForInteractiveLogin` end up registering for invocations that previously had all of their requested permissions silently dropped. Because dropping them left no permissions at all, those cmdlets fell back to their default permission set, so an app registered with only delegated permissions in `-Scopes`, or with only `-O365ManagementApplicationPermissions` or `-O365ManagementDelegatePermissions`, silently received the default permissions instead of the requested ones. Such an invocation now registers exactly the permissions asked for, which means the resulting app differs from before for the same command line. [#4178](https://github.com/pnp/powershell/issues/4178)
- Removed the Microsoft Graph permissions that Microsoft Graph no longer exposes from the permissions `Register-PnPEntraIDApp` accepts, as requesting them registered an app with a permission id Microsoft Graph does not recognise. `TeamsApp.Read`, `TeamsApp.Read.All`, `TeamsApp.ReadWrite` and `TeamsApp.ReadWrite.All` were replaced by the `TeamsAppInstallation.*` permissions, `Approval.Read.All` and `Approval.ReadWrite.All` by the `ApprovalSolution.*` permissions, and `Place.Read`, `Place.Read.Shared` and `Place.ReadWrite` by `Place.Read.All` and `Place.ReadWrite.All`. `ChannelMessage.Delete`, `DataLossPreventionPolicy.Evaluate`, `SensitiveInfoType.Detect`, `SensitiveInfoType.Read.All` and the eight `ApprovalRequest.*` permissions were removed without a replacement. [#4178](https://github.com/pnp/powershell/issues/4178)
...
### Fixed
- Fixed the Microsoft Graph `Sites.Selected` application permission being listed twice, once with an id that does not exist in Microsoft Graph. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed the SharePoint `EnterpriseResource.Read` delegated permission being listed twice, once with an id that SharePoint does not expose. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed the id of the Microsoft Graph `Contacts-OnPremisesSyncBehavior.ReadWrite.All` permission, both as a delegated and as an application permission. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `Register-PnPEntraIDApp -Scopes` and `Register-PnPEntraIDAppForInteractiveLogin -Scopes` silently dropping every delegated permission, such as `SPO.AllSites.FullControl` and `MSGraph.Directory.AccessAsUser.All`, as only application permissions were looked up. A permission that cannot be resolved is now reported as an error instead of being dropped. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `-O365ManagementApplicationPermissions` and `-O365ManagementDelegatePermissions` being accepted but never applied to the app registration, on both `Register-PnPEntraIDApp` and `Register-PnPEntraIDAppForInteractiveLogin`. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `Register-PnPEntraIDApp` and `Register-PnPEntraIDAppForInteractiveLogin` silently ignoring the per resource permission parameters when the obsolete `-Scopes` was specified as well. The combination is now reported as an error. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `Register-PnPEntraIDApp -ResourcePermissions` silently requesting no permissions for an entry holding an unsupported key, such as `ApplicationPermission` instead of `ApplicationPermissions`, or holding no permission at all. Both are now reported as an error, before authenticating. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed the permissions shipping with the module being parsed once per cmdlet invocation, per dynamic parameter and per application reported on by `Get-PnPEntraIDAppPermission`. They are now parsed once, which takes parameter discovery on `Register-PnPEntraIDAppForInteractiveLogin` from 275ms to under 1ms. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `Get-PnPEntraIDAppPermission` not reporting the Office 365 Management APIs permissions of an app registration. [#4178](https://github.com/pnp/powershell/issues/4178)
- Fixed `Register-PnPEntraIDApp` and `Register-PnPEntraIDAppForInteractiveLogin` running the consent flow against SharePoint for apps that request no SharePoint and no Microsoft Graph permissions. Such an app now reports that admin consent has to be granted through the Entra ID portal. [#4178](https://github.com/pnp/powershell/issues/4178)
