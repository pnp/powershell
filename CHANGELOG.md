...
### Fixed
- Fixed `Connect-PnPOnline -FederatedIdentity` failing with an HTTP 401 on SharePoint after roughly an hour because its CSOM requests kept using the access token acquired at connect time, and made it work outside of the commercial cloud by honouring `-AzureEnvironment` for the Entra ID login endpoint, the Microsoft Graph endpoint and the token exchange audience, acquiring the token through MSAL so it is cached and renewed instead of minted on every call. [#5446](https://github.com/pnp/powershell/pull/5446)
