...
### Changed
- Changed `Get-PnPSiteTemplate`, `Get-PnPTenantTemplate`, `Invoke-PnPTenantTemplate` and `Export-PnPPage` to fail when `-Configuration` cannot be read, such as a path that does not exist or JSON that cannot be parsed, instead of ignoring it without reporting an error and then processing everything. This presented as settings like `includeItems` appearing to have no effect; scripts which appeared to work were never honouring their configuration. [#5392](https://github.com/pnp/powershell/issues/5392)
...
### Fixed
- Fixed `Get-PnPTenantTemplate` and `Invoke-PnPTenantTemplate` throwing a `NullReferenceException` on a `-Configuration` which could not be read, and all four provisioning cmdlets throwing one when `-Configuration` was passed `$null`, rather than reporting what was wrong with the value. [#5392](https://github.com/pnp/powershell/issues/5392)
