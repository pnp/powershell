...
- Fixed `Get-PnPFileVersion`, `Remove-PnPFileVersion` and `Restore-PnPFileVersion` mangling file names containing URL encoding sequences, in the same way as `Get-PnPFile` did. A file whose name literally holds a sequence such as `%20` could not be addressed, as `-Url` was always decoded first, which either failed to find the file or acted on a different file that happened to match the decoded name. The URL is now used exactly as provided when a file exists at that location, and only falls back to the decoded form when it does not, so URLs copied from a browser address bar keep working. Where both files exist, `-Url` now addresses the one whose name matches it literally instead of the decoded one. [#5010](https://github.com/pnp/powershell/issues/5010)
