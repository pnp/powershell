...
### Changed
- Changed `Get-PnPSiteTemplate`, `Get-PnPTenantTemplate`, `Invoke-PnPTenantTemplate` and `Export-PnPPage` to fail when `-Configuration` cannot be read, and to warn about a misspelled property or an unrecognized handler inside it, instead of ignoring either and processing everything. [#5434](https://github.com/pnp/powershell/pull/5434)
