...
- Fixed `Get-PnPFileVersion`, `Remove-PnPFileVersion`, `Restore-PnPFileVersion`, `Convert-PnPFile`, `Get-PnPFileAnalyticsData`, `Set-PnPFileCheckedIn`, `Set-PnPFileCheckedOut`, `Undo-PnPFileCheckedOut`, `Get-PnPFileRetentionLabel` and `Get-PnPFileSensitivityLabel` mangling file names containing URL encoding sequences, in the same way as `Get-PnPFile` did. A file whose name literally holds a sequence such as `%20` could not be addressed, as `-Url` was always decoded first, which either failed to find the file or acted on a different file that happened to match the decoded name. The URL is now used exactly as provided when a file exists at that location, and only falls back to the decoded form when it does not, so URLs copied from a browser address bar keep working. Where both files exist, `-Url` now addresses the one whose name matches it literally instead of the decoded one. [#5010](https://github.com/pnp/powershell/issues/5010)
- Fixed `Get-PnPFile -AsFile` and `Get-PnPFile -AsMemoryStream` still mangling a file name containing a URL encoding sequence after [#5413](https://github.com/pnp/powershell/pull/5413), which resolved the URL correctly but then handed it to an API that rewrites `%20` back to a space. These two parameter sets, and the other cmdlets above which use the same API, now address such a file by its unique id instead of by its URL. [#5010](https://github.com/pnp/powershell/issues/5010)
- Fixed `Get-PnPFileRetentionLabel` and `Get-PnPFileSensitivityLabel` refusing a `-Url` relative to the web, such as `Shared Documents/file.docx`. Both already resolved it against the server relative URL of the web and then discarded the result, passing the unresolved URL to SharePoint. [#5010](https://github.com/pnp/powershell/issues/5010)
