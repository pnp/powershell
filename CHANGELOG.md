...
### Fixed
- Fixed `Get-PnPUser -WithRightsAssigned` timing out or exhausting its retries on sites with large user information lists. The cmdlet now retrieves only directly assigned principals and members of SharePoint groups that have role assignments, instead of loading every site user and every site group member before filtering. [#3162](https://github.com/pnp/powershell/issues/3162)
