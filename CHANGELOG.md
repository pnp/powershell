...
- Added `-IncludeExtensionAttributes` to `Get-PnPMicrosoft365Group`, which populates the `OnPremisesExtensionAttributes` property with the extension attributes 1-15 of a group. Microsoft Graph only populates these for groups that are synchronized from an on-premises Active Directory, not for cloud only groups. [#4091](https://github.com/pnp/powershell/issues/4091)
