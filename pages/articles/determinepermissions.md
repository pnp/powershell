...
| `NotApplicable` | The cmdlet needs no permissions on the application registration you connect with. That covers cmdlets which call no API at all, i.e. `Get-PnPChangeLog`, as well as cmdlets which authenticate separately, i.e. `Register-PnPEntraIDApp`, which can still require rights of their own such as a directory role. Read `Guidance` to see which applies. |
